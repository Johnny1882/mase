...
def plot_comparison_across_input_sizes(results_by_size, save_path=None):
    """Plot comparison of benchmark results across different input sizes."""
    # Get all metrics and input sizes
    input_sizes = sorted(results_by_size.keys())
    metrics = {
        "Tokens per Second": "avg_tokens_per_second",
        "Latency per Token (ms)": "avg_per_token_latency_ms",
        "Memory Usage (MB)": "avg_memory_allocated_mb",
        "KV Cache Size (MB)": "avg_kv_cache_size_mb"
    }
    
    # Create figure with subplots
    fig, axs = plt.subplots(2, 2, figsize=(15, 12))
    fig.suptitle("Model Performance Across Different Input Sizes", fontsize=16)
    ...
    for i, (metric_name, metric_key) in enumerate(metrics.items()):
        ...
        # Extract data for before and after transformation
        before_values = [results_by_size[size]['before'][metric_key] for size in input_sizes]
        after_values = [results_by_size[size]['after'][metric_key] for size in input_sizes]
        
        # Calculate improvement percentages
        if metric_name == "Latency per Token (ms)":
            # For latency, lower is better
            improvements = [(before - after) / before * 100 if before > 0 else 0 
        ...
        for j, imp in enumerate(improvements):
            ax.annotate(f'{imp:.1f}%', 
                       xy=(x[j], max(before_values[j], after_values[j])), 
                       xytext=(0, 5),
    ...
    # Create a separate figure for relative improvements
    fig_imp, ax_imp = plt.subplots(figsize=(12, 8))
    fig_imp.suptitle("Relative Improvement After Transformation by Input Size", fontsize=16)
    
    # Plot improvement percentages for each metric
    for metric_name, metric_key in metrics.items():
        ...
        ax_imp.plot(input_sizes, improvements, marker='o', label=metric_name)
    
    ax_imp.set_xlabel('Input Size (tokens)')
    ax_imp.set_ylabel('Improvement (%)')
    ax_imp.legend()
    ax_imp.grid(True, linestyle='--', alpha=0.7)
    
    if save_path:
        imp_save_path = save_path.replace('.png', '_improvements.png')
        plt.savefig(imp_save_path)
        print(f"Improvements figure saved to {imp_save_path}")
    
    return fig, fig_imp

def main():
    # Configuration
    model_path = "meta-llama/Llama-3.2-1B"  # Can be changed to other model sizes
    device = "cuda:0" if torch.cuda.is_available() else "cpu"
    num_samples = 10  # Reduced sample size for quicker testing
    input_sizes = [50, 100, 200, 500]  # Different input sizes to test
    ...
    # Store results for different input sizes
    results_by_size = {}
    ...
    for max_tokens in input_sizes:
        print_separator()
        print(f"BENCHMARKING WITH INPUT SIZE: {max_tokens} tokens")
    ...
    for size in input_sizes:
        for stage in ['before', 'after']:
            for metric, value in results_by_size[size][stage].items():
    ...
    results_df.to_csv("benchmark_results_by_size.csv", index=False)
    print("Results saved to benchmark_results_by_size.csv")
    
    # Plot comparison across input sizes
    plot_comparison_across_input_sizes(results_by_size, "benchmark_results_by_size.png")
    plt.show()
