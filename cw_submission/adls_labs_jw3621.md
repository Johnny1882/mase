...
We can also see that QAT model sometimes can exceed original baseline accuracy (red line). Quantization steps remove redundencies, and further training enable the network to learn parameter configurations that are more robust, and end up with a new model that generalize slightly better than the original model.
...
On the CPU, the fused SDPA kernel runs significantly faster compared to naive implementation. The same pattern can be observed on the GPU, as shown in the comparison diagram below. 
<!-- The speedup is due to the reduced memory access, as kernel fusion fused multiple operations together.  -->

![](img/lab4.2_fusekernel.png)

The below figures demostrate peak memory usage and time taken for native and fused kernel. For both kernel, we test using random Query, Key and Value with increasing sequence length. Each version is runned 50 times, with 10 times warmup.

The fused kernel shows lower peak memory usage, since multiple memory operations are combined into a single GPU kernel call. This gap becomes more significant when sequence length increase, as cost of loading data increase. This makes fused approach run faster because it reduces intermediate read/write operations, which is time-consuming.

![](img/lab4.2.kernelMemTest.png)

### Part 3 Custom Kernal

#### *(a) MXINT8 benefit for custom hardware*
...
#### *(b) The purpose of the variable dont_need_abs and bias*
...
MXINT has no implicit leading bits, hence after taking the absolute value, the highest bit of fraction parts can be seen as replacement. `dont_need_abs` indicates the 7-bit of fraction bits is 1 or 0. Fraction part value = frac/64, so when the bit = 1, fraction bits have value 1.xxx, 0.xxx otherwise. 
...
Mathematically, the formula for a BF16 number in **normal form**:
